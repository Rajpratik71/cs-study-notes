     ...
     {   
	  ...
	  for(int j = 1;j < i;j++)
	  {
	    if(A[j+1] < A[j])
	    {
	      ...
	      A[j] = A[j+1];
	      A[j+1] = w;
	      lastExchangeIndex = j;   //一趟排序中无序序列中最后一个记录的位置
	     }
	    }
	    i = lastExchangeIndex;
     }
...
{
     //对记录子序列A[low..high]进行一趟快速排序，并返回枢轴记录所在位置，
     //使得在它之前的记录的关键字均不大于它的关键字，在它之后的记录的关键字
     //均不小于它的关键字
     int pivotkey = A[low]; //这里选择首元素作为枢轴
...
把待排序的数组分成两半，对每一半进行排序，最后再把排序好的两半归并。这就是归并排序的基本思路
